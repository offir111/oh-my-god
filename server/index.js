    ...
    const response = await chatCompletionWithFallback(
      groqForApiRoutes(),
      {
        ...
        messages: [
          {
            role: 'system',
            content: `אתה מומחה תנ"ך ומורה מנוסה. המשתמש שואל שאלה על התנ"ך.

תחזיר תשובה עשירה בשדות הבאים:

1. **summary** — משפט אחד קצר (עד 12 מילים) שמסכם את הנושא — כמו תווית על ספר.
...
4. **didYouKnow** — עובדה מעניינת קצרה (משפט אחד) שקשורה לנושא — משהו שמרחיב את ההבנה.
5. **relatedTopics** — מערך של 3 נושאים קשורים קצרים (עד 4 מילים כל אחד) שהמשתמש יכול לחפש אחריהם.
6. **results** — פסוקים מהתנ"ך עם ציון רלוונטיות.

סוגי שאילתות לפי searchType:
- **why** (למה, מדוע): 4-6 פסוקים שמסבירים את הסיבה, relevanceScore גבוה לפסוקים ישירים
...
{"summary":"תיאור קצר בעד 12 מילים","searchType":"event","explanation":"הסבר 3-5 משפטים...","didYouKnow":"עובדה מעניינת...","relatedTopics":["נושא קשור א","נושא קשור ב","נושא קשור ג"],"results":[{"ref":"בראשית יב:י","text":"הפסוק המלא בעברית מקראית","context":"הסבר קצר","relevanceScore":5}]}

כללים:
- ref בפורמט: "שם ספר פרק:פסוק" בגמטריה עברית (למשל: "בראשית יב:י")
- relevanceScore: מספר 1-5 (5=הכי רלוונטי) — מיין תוצאות מ-5 עד 1
