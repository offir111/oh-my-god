...
export default function FaithChatPanel() {
  ...
  }, [publicMessages, dmSlots, activePanel, partnerTyping]);
    ...
    <>
      <style>{`
        ...
        .fcp-send {
          ...
          font-size: 0.76rem;
          cursor: pointer;
          background: linear-gradient(165deg, rgba(251,191,36,0.3), rgba(99,102,241,0.45));
          color: #f8fafc;
          flex-shrink: 0;
          align-self: stretch;
        }
        ...
        @media (max-width: 820px) {
          ...
          .fcp-main { order: 1; }
          .fcp-sidebar { order: 2; max-height: none; }
          .fcp-user-list { max-height: min(38vh, 320px); }
        }
      `}</style>
      ...
      <div className="fcp-root">
        <div className="fcp-topbar">
          <div className="fcp-status">
            ...
            {chatConnected
              ? joinedLobby
                ? `בצ׳אט · מזהה: ${mySocketId ? mySocketId.slice(0, 8) : '…'}`
                : 'מחובר לשרת — לחצו «התחבר לצ׳אט» למטה'
              : 'מתחבר לשרת…'}
          </div>
          <div className="fcp-name-field">
            {hasLoginChatName ? (
            ...
            <div className="fcp-grid">
              <aside className="fcp-sidebar" aria-label="מחוברים כעת">
                <div className="fcp-side-title">
                  מחוברים עכשיו
                  <span className="fcp-count">{othersOnline.length + 1}</span>
                </div>
                  ...
                  {othersOnline.map(u => (
                      <div key={u.socketId} className="fcp-user-row">
                        ...
                        <button
                          ...
                          disabled={!chatConnected || !!dmPartner}
                          onClick={() => requestDm(u.socketId)}
                          title={dmPartner ? 'סיים שיחה נוכחית לפני בקשה חדשה' : 'שלח בקשת שיחה פרטית'}
                        >
                          שיחה
                        </button>
                      </div>
                  ...
                  {activePanel === 'public' ? (
                    publicMessages.length === 0 ? (
                      ...
                      publicMessages.map(m => {
                        const mine = m.fromSocketId === mySocketId || (!!faithSocket.id && m.fromSocketId === faithSocket.id);
                        return (
                          <article key={m.id}>
                            <div
                            ...
                            >
                              ...
                              <div className="fcp-msg-block">
                                <span className="fcp-msg-user">{m.displayName}{mine ? ' (את/ה)' : ''}</span>
